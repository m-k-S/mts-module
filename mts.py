...
print("Loaded MyComponents from", __file__)
...
def MTS(x=0, y=0, angle=0, thumbscrews=True):
	...
	baseplate.place_element_along_beam("HWP", optomech.waveplate, beam,
	                               beam_index=0b1, distance=0.5*layout.inch, angle=layout.cardinal['right'],
								   mount_type=optomech.rotation_stage_rsp05, mount_args=dict(adapter=True, adapter_args=dict(drill=True)))
	
	baseplate.place_element_along_beam("Beam_Splitter", optomech.cube_splitter, beam,
                                       beam_index=0b1, distance=1.5*layout.inch, angle=layout.cardinal['up'], invert=False)
	
	baseplate.place_element_along_beam("Mirror_1", optomech.circular_mirror, beam,
	                               beam_index=0b11, distance=2.5*layout.inch, angle=layout.turn['down-right'],
								   mount_type=optomech.mirror_mount_km05, mount_args=dict(drill=True, thumbscrews=True))
	...
	baseplate.place_element_along_beam("Mirror_2", MyComponents.CircularMirror, beam,
	                               beam_index=0b11, distance=5*layout.inch, angle=layout.turn['right-up'],
	                               mount_type=optomech.mirror_mount_km05, mount_args=dict(drill=True, thumbscrews=True))
	...
	baseplate.place_element_along_beam("Beam_Splitter_2", optomech.cube_splitter, beam,
	                               beam_index=0b11, distance=2.5*layout.inch, angle=layout.cardinal['up'], invert=False)

	baseplate.place_element_along_beam("Photodetector", MyComponents.Photodetector, beam,
	                               beam_index=0b100, distance=2*layout.inch, angle=layout.cardinal['left'])
